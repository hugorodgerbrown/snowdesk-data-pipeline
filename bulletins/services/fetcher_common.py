"""
bulletins/services/fetcher_common.py — Shared helpers for bulletin fetcher services.
...
2. **NDJSON stash writers** — ``merge_ndjson_records`` (dedupe + sort) and
   ...
   stash writer delegates to ``bulletins.services.slf_archive`` which has its
...
def write_ndjson_archive(
    ...
    *,
    ...
    handled separately in ``bulletins.services.slf_archive`` to preserve
