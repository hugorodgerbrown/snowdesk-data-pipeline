...
// SNOW-610: the shared helper block that used to sit here — INT_TO_RATING,
// DATE_KEY_RE / readUrlDateParam, the localStorage trio, the popup date and
// relative-time formatters, getSeasonRatings, fetchBulletinGroupingsForDate
// and repaintRegionsForDate — now lives in static/js/map_shared.js, which
// loads immediately before this file. `RATINGS_URL` and
// `BULLETIN_GROUPINGS_URL_MODULE` are declared there and written by the
// main IIFE below, once it has read the #map element's data-* attributes.
...
// SNOW-610: the two download controls that used to sit here —
// `mapDownloadControlInit` (the per-region roundel in the ribbon header)
// and `mapCustomDownloadControlInit` (the framing overlay) — now live in
// static/js/map_region_download.js and static/js/map_custom_download.js,
// which load after this file so their IIFEs still run in the order they
// did here. Both reach the shared download machinery in
// static/js/map_basemap_downloads.js.
