"""
core/management/commands/sync_waffle_flags.py — Reconcile Flags to a manifest.

Replaces the old "one data migration per flag" pattern (see
docs/feature-flags.md) with a single manifest at
``core/fixtures/waffle_flags.json``. The command diffs the manifest against
...
class Command(BaseCommand):
    """Reconcile waffle.Flag rows to core/fixtures/waffle_flags.json.
    ...
    help = (
        ...
        "core/fixtures/waffle_flags.json by create + delete only (never "
