...
| Public explainers — self-contained HTML pages written for readers WITHOUT repo access, so no ticket ids, file paths or decision-doc references. "From GPX to Line" covers the upload pipeline (what a GPX holds, the figures, Douglas-Peucker thinning, what is stored and drawn); "Anatomy of a Route" covers the six layers a saved route carries (terrain, slope under the line, key passages, no-fall passages, fall line, the day's bulletin) and keeps its screenshots in `img/anatomy-of-a-route/`; "Four Ways to Draw the Fall Line" walks four candidate drawings of the track's relationship to the fall line and records why each was put down; "Pitch and Roll on a Ski Track" is the frame that replaced them — tan²(pitch) + tan²(roll) = tan²(slope), so the profile already draws pitch and only the bank angle needs a mark, drawn as the bank ribbon. Both render live from one real track. "The Track Cannot Be Steeper Than the Ground" explains the along-track gradient: the physical bound that a track's gradient never exceeds the ground's angle, the recording break on the Backside tour it catches, the 5° rejection rule, and what a gradient read from stored points (the original GPX is not kept) can never be better than — live from the four canonical tours. Script-drawn figures are marked `data-baked` and written into the HTML by `bin/bake-explainer` (jsdom, idempotent), so a page reads with JavaScript off; re-run it after editing a page's script or data, and colour figures with `var(--token)`, never resolved values | [`docs/explainers/`](docs/explainers/) |
...
| Why a saved route on the map is drawn as its legs, not in slope classes (route_legs_core.js, routes-leg-climb / routes-leg-descent, routes-transitions, point_from / point_to, bindLegDimming; the gap until SNOW-1019) | [`docs/decisions/legs-not-slope-classes-on-the-map.md`](docs/decisions/legs-not-slope-classes-on-the-map.md) |
| Why the bank angle is drawn signed (bank.py, bank_angle_deg, bank_angles, `roll_deg`, `banks` on the wire, bank_ribbon_core.js bankWedges, the level-ski wedges; lookup vs pattern, no gate, the flat array) | [`docs/decisions/the-bank-angle-is-drawn-signed.md`](docs/decisions/the-bank-angle-is-drawn-signed.md) |
...
| Why a what3words address is stored indefinitely (Location.what3words, three_word_address, convert-to-3wa vs convert-to-coordinates, the pin-move invalidation) | [`docs/decisions/what3words-addresses-are-stored-indefinitely.md`](docs/decisions/what3words-addresses-are-stored-indefinitely.md) |
| Why a field observation's address is owner-only (observation_list vs community_reports_geojson, the 3m square vs the rounded coordinate) | [`docs/decisions/an-address-is-owner-only-on-observations.md`](docs/decisions/an-address-is-owner-only-on-observations.md) |
...
| Why a grouping outline asserts an aggregation (BulletinGrouping, MIN_GROUPED_REGIONS, degenerate(), purge_degenerate_bulletin_groupings; why a day can return zero features) | [`docs/decisions/a-grouping-outline-asserts-an-aggregation.md`](docs/decisions/a-grouping-outline-asserts-an-aggregation.md) |
...
| Why a routine SW update applies silently and the banner is only for a stuck worker (queueSilentUpdate, applyWaitingWorker at once when `pwa-shell` matches (a fresh tab) or on visibilitychange→hidden, workerIsStuck, `shell-identity`, why BUILD_IDENTITY and the versioned copy were removed) | [`docs/decisions/service-worker-updates-apply-silently.md`](docs/decisions/service-worker-updates-apply-silently.md) |
...
| Worktree DB seeding, why migrate + sync_waffle_flags rerun every session, dev credentials, seed_test_data + seed_test_week (golden week) dataset coverage | [`docs/worktrees.md`](docs/worktrees.md) |
