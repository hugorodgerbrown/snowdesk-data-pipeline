...
| Public explainers — self-contained HTML pages written for readers WITHOUT repo access, so no ticket ids, file paths or decision-doc references. "From GPX to Line" covers the upload pipeline (what a GPX holds, the figures, Douglas-Peucker thinning, what is stored and drawn); "Anatomy of a Route" covers the six layers a saved route carries (terrain, slope under the line, key passages, no-fall passages, fall line, the day's bulletin) and keeps its screenshots in `img/anatomy-of-a-route/`; "Four Ways to Draw the Fall Line" is the argument for showing the ground's angle against the track's own gradient rather than naming the relationship in words, and draws all four candidates live from one real track | [`docs/explainers/`](docs/explainers/) |
