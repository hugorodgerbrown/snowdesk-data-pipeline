...
Walks the ALBINA archive at ``static.avalanche.report/bulletins/YYYY-MM-DD/``
and pulls every EUREGIO regional CAAMLv6 bulletin (AT-07, IT-32-BZ, IT-32-TN)
for the date range, then writes one bulletin per line to
``sample_data/euregio_archive.ndjson`` ascending by ``validTime.startTime``.

The archive mirrors the shape of ``sample_data/slf_archive.ndjson`` so the
existing ``load_euregio_bulletin --file`` path (with ndjson support) can
seed a dev DB from a single committed artefact, with no network required.

This is a one-off committed script — not a management command. ALBINA's
archive is append-only and indexed by date directory, so a single full
season fetch is enough to populate a season's worth of dev data.
Re-running the script overwrites the output file; an incremental
``--since DATE`` mode could be added if it ever needs to run more than
once per season.

Usage:
    python scripts/fetch_euregio_archive.py
    python scripts/fetch_euregio_archive.py \
        --start-date 2025-11-01 --end-date 2026-05-13
...
DEFAULT_OUTPUT = REPO_ROOT / "sample_data" / "euregio_archive.ndjson"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Fetch a season of EUREGIO bulletins into a local NDJSON archive.",
    )
    parser.add_argument(
        "--start-date",
