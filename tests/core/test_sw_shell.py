...
class TestRealShellSource:
    ...
    def test_committed_sw_js_carries_no_build_identity(self) -> None:
        """No per-deploy value but the cache name is written into the worker.

        SNOW-933's ``BUILD_IDENTITY`` made the worker's bytes differ on
        every deploy; SNOW-1025 removed it. A placeholder reappearing here
        would be the first half of putting that back.
        """
        source = sw_shell.SW_JS_PATH.read_text(encoding="utf-8")

        assert "BUILD_IDENTITY" not in source
