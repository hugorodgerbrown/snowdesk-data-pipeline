...
class TestRule1DangerousConditions:
    ...
    def test_danger_4_returns_dangerous(self) -> None:
        ...
        assert compute_day_character(_render_model("4")).label == "Dangerous conditions"

    def test_danger_5_returns_dangerous(self) -> None:
        """Danger level 5 → Dangerous conditions."""
        assert compute_day_character(_render_model("5")).label == "Dangerous conditions"
    ...
    def test_danger_4_ignores_problems(self) -> None:
        ...
        assert compute_day_character(rm).label == "Dangerous conditions"
...
class TestRule3WidespreadDanger:
    ...
    def test_not_triggered_at_danger_2(self) -> None:
        ...
        assert compute_day_character(rm).label == "Manageable day"
...
class TestRule5StableDay:
    ...
    def test_danger_1_is_stable(self) -> None:
        """Danger 1 → Stable day."""
        assert compute_day_character(_render_model("1")).label == "Stable day"
...
class TestSafeDefault:
    ...
    def test_empty_render_model_defaults_to_stable(self) -> None:
        ...
        assert compute_day_character({}).label == "Stable day"
...
class TestDayCharacterInPanelContext:
    ...
    def test_panel_context_contains_day_character(self) -> None:
        ...
        assert ctx["day_character"].label == "Hard-to-read day"
        assert ctx["day_character"].explainer


class TestExplainerPerLabel:
    """
    Each cascade branch ships a non-empty explainer alongside the label.

    The explainer copy is a free-text presentation detail; we assert
    presence (truthy) per branch rather than exact wording so future
    copy edits don't require test churn. Coverage of the underlying
    rule cascade lives in the dedicated TestRule* classes above.
    """

    def test_dangerous_explainer_is_populated(self) -> None:
        """Dangerous conditions branch returns a non-empty explainer."""
        assert compute_day_character(_render_model("4")).explainer

    def test_hard_to_read_explainer_is_populated(self) -> None:
        """Hard-to-read branch returns a non-empty explainer."""
        rm = _render_model("2", problems=[_problem("persistent_weak_layers")])
        assert compute_day_character(rm).explainer

    def test_widespread_explainer_is_populated(self) -> None:
        """Widespread branch returns a non-empty explainer."""
        rm = _render_model("3", problems=[_problem("new_snow", lower=1800)])
        assert compute_day_character(rm).explainer

    def test_manageable_explainer_is_populated(self) -> None:
        """Manageable branch returns a non-empty explainer."""
        rm = _render_model("2", problems=[_problem("new_snow")])
        assert compute_day_character(rm).explainer
