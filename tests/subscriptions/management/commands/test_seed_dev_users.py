"""
tests/subscriptions/management/commands/test_seed_dev_users.py — Tests for the
...
from subscriptions.management.commands.seed_dev_users import (
    NORMAL_USER_EMAIL,
    PASSWORD,
...
@pytest.fixture()
def ch4115_region(db: None) -> object:
...
class TestSeedDevUsersSuperuser:
    ...
    def test_superuser_created(self, ch4115_region: object) -> None:
    ...
    def test_superuser_email_lowercased(self, ch4115_region: object) -> None:
        """The superuser email is stored in lowercase."""
        call_command("seed_dev_users", verbosity=0)
        user = User.objects.get(username=SUPERUSER_EMAIL.lower())
        assert user.email == SUPERUSER_EMAIL.lower()

    def test_superuser_has_usable_password(self, ch4115_region: object) -> None:
...
class TestSeedDevUsersNormalUser:
    ...
    def test_normal_user_has_subscriber_profile(self, ch4115_region: object) -> None:
    ...
    def test_normal_user_email_lowercased(self, ch4115_region: object) -> None:
    ...
    def test_normal_user_has_usable_password(self, ch4115_region: object) -> None:
        """The normal user can authenticate with the dev password."""
        call_command("seed_dev_users", verbosity=0)
        user = User.objects.get(username=NORMAL_USER_EMAIL.lower())
        assert user.check_password(PASSWORD)

    def test_normal_user_subscription_to_ch4115(self, ch4115_region: object) -> None:
...
class TestSeedDevUsersIdempotency:
    ...
    def test_running_twice_no_duplicate_users(self, ch4115_region: object) -> None:
    ...
    def test_password_still_valid_after_second_run(self, ch4115_region: object) -> None:
...
class TestSeedDevUsersProductionRefusal:
    ...
    def test_raises_command_error_when_debug_false(self, ch4115_region: object) -> None:
        """seed_dev_users raises CommandError when DEBUG=False."""
        with pytest.raises(CommandError):
            call_command("seed_dev_users", verbosity=0)

    def test_does_not_raise_when_debug_true(self, ch4115_region: object) -> None:
