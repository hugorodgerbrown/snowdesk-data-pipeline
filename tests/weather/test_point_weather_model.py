"""
tests/weather/test_point_weather_model.py — Tests for the ForecastCellWeather model.

Covers:
  - Factory produces a valid instance via .create().
  - to_string() / __str__() format.
  - ForecastCellWeatherQuerySet.for_date() filter.
  - ForecastCellWeatherQuerySet.forecast_for_point() filter/ordering
    (SNOW-417).
  - freezing_level_height / hourly_series persistence (SNOW-417).
  - unique_together constraint raises IntegrityError on duplicate
    (forecast_cell, date).
  - ordering: newest date first, then forecast_cell__id ascending.
  - Deleting the linked ForecastCell cascades to ForecastCellWeather.
"""
...
from tests.factories import ForecastCellFactory, ForecastCellWeatherFactory


@pytest.mark.django_db
class TestForecastCellWeatherFactory:
    """The factory produces valid, well-formed ForecastCellWeather instances."""

    def test_create_returns_forecast_cell_weather(self) -> None:
        """ForecastCellWeatherFactory.create() returns a persisted row."""
        weather = ForecastCellWeatherFactory.create()
        assert isinstance(weather, ForecastCellWeather)
    ...
    def test_defaults_are_sensible(self) -> None:
        ...
        assert weather.forecast_cell is not None
    ...
    def test_override_weather_code(self) -> None:
        ...
        weather = ForecastCellWeatherFactory.create(weather_code=61)
...
@pytest.mark.django_db
class TestForecastCellWeatherStr:
...
@pytest.mark.django_db
class TestForecastCellWeatherQuerySet:
    """ForecastCellWeatherQuerySet.for_date() filters correctly."""
    ...
    def test_for_date_returns_matching_rows(self) -> None:
        ...
        weather_target = ForecastCellWeatherFactory.create(valid_for_date=target)
        ForecastCellWeatherFactory.create(
            forecast_cell=ForecastCellFactory.create(latitude=47.0, longitude=8.0),
            valid_for_date=other,
        )

        result = ForecastCellWeather.objects.for_date(target)
    ...
    def test_for_date_empty_when_no_match(self) -> None:
        ...
        ForecastCellWeatherFactory.create(valid_for_date=datetime.date(2026, 4, 30))
        result = ForecastCellWeather.objects.for_date(datetime.date(2026, 5, 1))
        assert result.count() == 0


@pytest.mark.django_db
class TestForecastCellWeatherForecastForPoint:
    """ForecastCellWeatherQuerySet.forecast_for_point() filters and orders correctly."""
    ...
    def test_filters_to_one_point_from_start_date(self) -> None:
        ...
        point = ForecastCellFactory.create()
        other_point = ForecastCellFactory.create(latitude=47.0, longitude=8.0)
        start = datetime.date(2026, 5, 1)
        before = ForecastCellWeatherFactory.create(
            forecast_cell=point, valid_for_date=start - datetime.timedelta(days=1)
        )
        on_start = ForecastCellWeatherFactory.create(
            forecast_cell=point, valid_for_date=start
        )
        after = ForecastCellWeatherFactory.create(
            forecast_cell=point, valid_for_date=start + datetime.timedelta(days=1)
        )
        ForecastCellWeatherFactory.create(
            forecast_cell=other_point, valid_for_date=start
        )

        result = list(ForecastCellWeather.objects.forecast_for_point(point, start))
    ...
    def test_orders_ascending_by_date(self) -> None:
        ...
        point = ForecastCellFactory.create()
        start = datetime.date(2026, 5, 1)
        day2 = ForecastCellWeatherFactory.create(
            forecast_cell=point, valid_for_date=start + datetime.timedelta(days=2)
        )
    ...
    def test_empty_when_no_rows_at_or_after_start(self) -> None:
        ...
        result = ForecastCellWeather.objects.forecast_for_point(point, start)

        assert result.count() == 0


@pytest.mark.django_db
class TestForecastCellWeatherConstraints:
    """Model-level integrity constraints."""

    def test_unique_together_point_date(self) -> None:
        """Inserting two rows for the same (forecast_cell, date) raises IntegrityError."""
        point = ForecastCellFactory.create()
        target = datetime.date(2026, 5, 1)
        ForecastCellWeatherFactory.create(forecast_cell=point, valid_for_date=target)
    ...
    def test_different_points_same_date_allowed(self) -> None:
        ...
        point_a = ForecastCellFactory.create()
        point_b = ForecastCellFactory.create(latitude=47.0, longitude=8.0)
        target = datetime.date(2026, 5, 1)
        ForecastCellWeatherFactory.create(forecast_cell=point_a, valid_for_date=target)
        weather_b = ForecastCellWeatherFactory.create(
            forecast_cell=point_b, valid_for_date=target
        )
    ...
    def test_same_point_different_dates_allowed(self) -> None:
        ...
        point = ForecastCellFactory.create()
        weather1 = ForecastCellWeatherFactory.create(
            forecast_cell=point, valid_for_date=datetime.date(2026, 5, 1)
        )
        weather2 = ForecastCellWeatherFactory.create(
            forecast_cell=point, valid_for_date=datetime.date(2026, 5, 2)
        )
        assert weather1.pk != weather2.pk

    def test_cascade_delete_from_forecast_cell(self) -> None:
        """Deleting the linked ForecastCell also deletes the ForecastCellWeather."""
        point = ForecastCellFactory.create()
        weather = ForecastCellWeatherFactory.create(forecast_cell=point)
        weather_pk = weather.pk
        point.delete()
        assert not ForecastCellWeather.objects.filter(pk=weather_pk).exists()


@pytest.mark.django_db
class TestForecastCellWeatherOrdering:
    """Default ordering: newest date first, then forecast_cell__id ascending."""

    def test_ordering_newest_date_first(self) -> None:
        """Queryset returns most recent valid_for_date first."""
        point = ForecastCellFactory.create()
        weather_old = ForecastCellWeatherFactory.create(
            forecast_cell=point, valid_for_date=datetime.date(2026, 4, 30)
        )
        weather_new = ForecastCellWeatherFactory.create(
            forecast_cell=point, valid_for_date=datetime.date(2026, 5, 1)
        )
        qs = list(ForecastCellWeather.objects.all())
        assert qs[0] == weather_new
        assert qs[1] == weather_old


@pytest.mark.django_db
class TestForecastCellWeatherTzAware:
