"""
tests/weather/test_weather_snapshot_admin.py — Tests for WeatherSnapshotAdmin.
...
FETCH_URL = reverse("admin:weather_weathersnapshot_fetch_today")
...
class TestFetchTodayPost:
    ...
    def test_post_calls_fetcher_and_shows_success_message(
        ...
        with patch(
            "apps.weather.admin.fetch_all_regions", return_value=counts
        ) as mock_fn:
    ...
    def test_post_with_failures_shows_warning(self, staff_client: Client) -> None:
        ...
        with patch("apps.weather.admin.fetch_all_regions", return_value=counts):
            response = staff_client.post(FETCH_URL, follow=True)
    ...
    def test_post_exception_shows_error_message(self, staff_client: Client) -> None:
        ...
        with patch(
            "apps.weather.admin.fetch_all_regions", side_effect=RuntimeError("boom")
        ):
            response = staff_client.post(FETCH_URL, follow=True)
...
class TestFetchTodayGet:
    ...
    def test_get_is_rejected(self, staff_client: Client) -> None:
        ...
        with patch("apps.weather.admin.fetch_all_regions") as mock_fn:
