"""
tests/weather/test_forecast_history_model.py — Tests for ForecastCellWeatherHistory.

Covers:
  - Factory produces a valid instance via .create().
  - to_string() / __str__() format, including the signed lead-days marker.
  - ForecastCellWeatherHistoryQuerySet.convergence_for() filter/ordering —
  ...
  - unique_together constraint raises IntegrityError on a duplicate
    (forecast_cell, valid_for_date, issued_date).
  ...
  - Deleting the linked ForecastCell cascades.
"""
...
class TestForecastCellWeatherHistoryFactory:
    """The factory produces valid, well-formed instances."""

    def test_create_returns_history_row(self) -> None:
        """ForecastCellWeatherHistoryFactory.create() returns a persisted row."""
        row = ForecastCellWeatherHistoryFactory.create()
        assert isinstance(row, ForecastCellWeatherHistory)
    ...
    def test_defaults_are_sensible(self) -> None:
        ...
        assert row.forecast_cell is not None
...
class TestToString:
    ...
    def test_to_string_format(self) -> None:
        ...
        expected = (
            f"{row.forecast_cell.to_string()} 2026-05-08 issued 2026-05-05 (+3d) wmo=2"
        )
...
class TestConvergenceFor:
    ...
    def test_returns_series_in_issue_order(self) -> None:
        ...
        series = ForecastCellWeatherHistory.objects.convergence_for(point, VALID_DAY)
    ...
    def test_excludes_other_days_and_points(self) -> None:
        ...
        point = ForecastCellFactory.create()
        # Distinct coordinates: ForecastCell is unique on the quantised
        # (lat_cell, lon_cell, elevation_band) triple, and the factory's
        # defaults are fixed.
        other_point = ForecastCellFactory.create(latitude=46.5, longitude=7.9)
        wanted = ForecastCellWeatherHistoryFactory.create(
            forecast_cell=point, valid_for_date=VALID_DAY
        )
        ForecastCellWeatherHistoryFactory.create(
            forecast_cell=point,
        ...
        ForecastCellWeatherHistoryFactory.create(
            forecast_cell=other_point, valid_for_date=VALID_DAY
        )
    ...
    def test_empty_for_unknown_day(self) -> None:
        ...
        point = ForecastCellFactory.create()
        assert not ForecastCellWeatherHistory.objects.convergence_for(
            point, VALID_DAY
        ).exists()
...
class TestUniqueness:
    """The key is (forecast_cell, valid_for_date, issued_date)."""
    ...
    def test_same_day_different_issue_dates_coexist(self) -> None:
        ...
        assert (
            ForecastCellWeatherHistory.objects.filter(
                forecast_cell=point, valid_for_date=VALID_DAY
            ).count()
            == 4
        )
...
class TestOrderingAndCascade:
    ...
    def test_default_ordering_newest_day_then_issue_ascending(self) -> None:
        ...
        point = ForecastCellFactory.create()
        older_day = ForecastCellWeatherHistoryFactory.create(
            forecast_cell=point,
            valid_for_date=VALID_DAY - datetime.timedelta(days=1),
            issued_date=datetime.date(2026, 5, 4),
        )
        later_issue = ForecastCellWeatherHistoryFactory.create(
            forecast_cell=point,
            valid_for_date=VALID_DAY,
            issued_date=datetime.date(2026, 5, 6),
        )
        earlier_issue = ForecastCellWeatherHistoryFactory.create(
            forecast_cell=point,
            valid_for_date=VALID_DAY,
            issued_date=datetime.date(2026, 5, 4),
        )

        assert list(ForecastCellWeatherHistory.objects.all()) == [
            earlier_issue,
            later_issue,
            older_day,
        ]
    ...
    def test_deleting_point_cascades(self) -> None:
        """Deleting the ForecastCell removes its history rows."""
        row = ForecastCellWeatherHistoryFactory.create()
        row.forecast_cell.delete()
        assert not ForecastCellWeatherHistory.objects.filter(pk=row.pk).exists()
