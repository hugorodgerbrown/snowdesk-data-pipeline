/*
 * tests/js/test_routes_offline.js — the routes panel's offline row store
 * (SNOW-950, static/js/routes_offline.js).
 *
 * The module is storage only: it writes what came back from ``routes:list``
 * and reads it out again. Everything it does is jsdom's to hold — one htmx
 * lifecycle event and an IndexedDB round trip — so this is the whole of its
 * coverage and none of it belongs in a browser. What ``routes.js`` then
 * paints from a hit is covered in tests/js/test_routes_panel.js.
 *
 ...
 * written here (SNOW-722's ownership lesson, which cost this very panel a
...
await import('../../static/js/routes_offline.js');

const STORE = 'data:panel_rows';
const KEY = 'routes';
const LIST_URL = '/routes/partials/list/?variant=map';
const ROWS = '<ul><li id="route-1">Haute Route</li></ul>';
...
beforeEach(async () => {
  ...
  window.pwaPanelRows.invalidate('observations');
  document.body.innerHTML =
    '<div data-routes-rows></div><div data-report-rows></div>' +
    '<div data-favourite-rows></div>';
  rows = document.querySelector('[data-routes-rows]');
});
...
describe('write-through', () => {
  ...
  it("does not persist the observations panel's rows", async () => {
    // SNOW-722: this listener is bound to document.body and is offered every
    // swap on the page. Another panel's rows arriving here would repaint the
    // routes panel with somebody else's list.
    replaySwap(document.querySelector('[data-report-rows]'));

    await new Promise((resolve) => setTimeout(resolve, 20));
    expect(await window.pwaDb.get(STORE, KEY)).toBeUndefined();
  });

  it("does not persist the favourites panel's rows", async () => {
    replaySwap(document.querySelector('[data-favourite-rows]'));
...
describe('the idle warm', () => {
  it('persists the rows without the panel ever being opened', async () => {
    ...
    // opened without it. routes.js warms at module init on every map page
...
describe('read back', () => {
  it('returns the cached rows', async () => {
    ...
    const record = await window.pwaRoutesOffline.read();
    expect(record.body).toBe(ROWS);
  });

  it('returns null when nothing is cached', async () => {
    expect(await window.pwaRoutesOffline.read()).toBeNull();
  });
...
describe('the public surface', () => {
  it('is frozen, like every other window.pwa* bridge', () => {
    expect(Object.isFrozen(window.pwaRoutesOffline)).toBe(true);
  });
});
