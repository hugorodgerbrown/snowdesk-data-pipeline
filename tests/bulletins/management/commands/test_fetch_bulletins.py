...
@pytest.mark.django_db
class TestFetchBulletinsLegacyUrl:
    """SNOW-900: pinning SLF back to the pre-2026/27 export.

    An environment variable rather than a flag, deliberately: the day SLF
    flip their unversioned endpoint, the pin has to be pullable on a live
    dyno without a deploy and without editing the cron entry.
    """

    @patch(PATCH_SLF)
    def test_an_empty_legacy_setting_leaves_the_live_path_alone(
        self, mock_run: MagicMock
    ) -> None:
        """The default is empty, so nothing changes until someone sets it."""
        mock_run.return_value = _make_successful_run()
    ...
    @patch(PATCH_SLF)
    def test_a_set_legacy_url_overrides_the_live_setting(
        self, mock_run: MagicMock
    ) -> None:
        ...
        _, kwargs = mock_run.call_args
        assert kwargs["base_url"] == "https://aws.slf.ch/api/bulletin/caaml/v3"

    @override_settings(
        SLF_API_LEGACY_URL="https://aws.slf.ch/api/bulletin/caaml/v3",
        SLF_API_LOCAL_MIRROR_URL="http://localhost:8000/dev/slf-mirror",
    )
    @patch(PATCH_SLF)
    def test_local_mirror_still_wins(self, mock_run: MagicMock) -> None:
        """An explicit flag outranks an ambient environment variable.

        Otherwise a legacy pin left set in a dev ``.env`` would silently
        send ``--local-mirror`` runs at the live SLF API.
        """
        mock_run.return_value = _make_successful_run()
        ...
        _, kwargs = mock_run.call_args
        assert kwargs["base_url"] == "http://localhost:8000/dev/slf-mirror"

    @override_settings(SLF_API_LEGACY_URL="https://aws.slf.ch/api/bulletin/caaml/v3")
    @patch(PATCH_ALBINA)
    def test_the_pin_is_slf_only(self, mock_run: MagicMock) -> None:
        """No other provider carries a legacy path, so none is affected."""
        mock_run.return_value = _make_successful_run()
