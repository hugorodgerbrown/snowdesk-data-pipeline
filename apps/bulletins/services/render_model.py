"""
apps/bulletins/services/render_model.py — Render model builder (SLF, ALBINA, MF).
...
Version 6 changes:
  - Wet-snow prose enrichment: when an SLF wet-snow or gliding-snow problem
    ...
    (``apps.bulletins.services.prose.parse_for``).  On a successful parse the
    ...
    ``apps/bulletins/services/prose/en.py`` silently falls back to the previous
...
Version 8 changes:
  - Removed the duplicate top-level ``snowpack_structure`` key (SNOW-488).
    All production consumers (``apps/public/templatetags/snowdesk_html.py``,
    ``apps/mcp_server/tools.py``) already read the nested
