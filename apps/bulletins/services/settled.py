"""
apps/bulletins/services/settled.py — Settled/unsettled boundary for cacheable dates.
...
(``apps.bulletins.services.slf_fetcher.get_sources``) that
...
``apps/public/api.py`` calls ``is_settled`` to decide the ``Cache-Control`` header
...
from apps.bulletins.services.slf_fetcher import get_sources
