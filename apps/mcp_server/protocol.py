"""
apps/mcp_server/protocol.py — JSON-RPC 2.0 envelope + MCP method router.
...
``dispatch()`` is the only entry point ``apps/mcp_server/views.py`` calls.
"""
...
class ProtocolError(Exception):
    ...
    Raised by handlers (in this module or in ``apps.mcp_server.tools``) for
...
def parse_error_response() -> dict[str, Any]:
    ...
    Used directly by ``apps.mcp_server.views.mcp_endpoint`` — a body that fails
...
def _invoke_handler(handler: Any, parsed: _ParsedRequest) -> dict[str, Any]:
    ...
    from the inline version (mirrors ``apps.analytics.views._parse_events``).
