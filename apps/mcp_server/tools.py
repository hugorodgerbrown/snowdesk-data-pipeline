"""
apps/mcp_server/tools.py — MCP tool registry and handlers.

Read-only tools, composed from services that already exist elsewhere in
the codebase (``apps.mcp_server.resolvers``, ``apps.public.views``,
...
``apps.mcp_server.protocol`` invokes via the ``TOOLS`` registry. Keeping the
...
class ToolError(Exception):
    ...
    Caught by ``apps.mcp_server.protocol._handle_tools_call`` and turned into an
...
def _handle_bulk_current_conditions(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    # Local import — see the module-level import in apps.mcp_server.protocol; a
    # top-level import here would create a cycle (protocol imports tools).
    from apps.mcp_server.protocol import INVALID_PARAMS, ProtocolError
