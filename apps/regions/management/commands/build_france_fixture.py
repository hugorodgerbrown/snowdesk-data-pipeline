"""build_france_fixture — build apps/regions/fixtures/eaws_FR.json from source data.
...
L4 names are resolved via ``apps.regions.names.lookup(..., "fr")`` from the
...
Usage:
    ...
    uv run python manage.py build_france_fixture

    # Write apps/regions/fixtures/eaws_FR.json.
    uv run python manage.py build_france_fixture --commit
"""
...
_FRANCE_FIXTURE = Path("apps/regions/fixtures/eaws_FR.json")
...
class Command(BaseCommand):
    """Build apps/regions/fixtures/eaws_FR.json from EAWS + MF source files.

    Read-only by default; pass ``--commit`` to write the fixture.
    """

    help = (
        "Build apps/regions/fixtures/eaws_FR.json from EAWS micro-region geometry "
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        write_fixture(_FRANCE_FIXTURE, entries)
