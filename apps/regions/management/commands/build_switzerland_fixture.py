"""build_switzerland_fixture — build apps/regions/fixtures/eaws_CH.json.
...
L4 names are resolved via ``apps.regions.names.lookup(region_id, "de")``,
...
using the shared helpers in ``apps.regions.fixture_utils``.
...
Usage:
    ...
    uv run python manage.py build_switzerland_fixture

    # Write apps/regions/fixtures/eaws_CH.json.
    uv run python manage.py build_switzerland_fixture --commit
"""
...
_CH_FIXTURE = _REPO_ROOT / "apps" / "regions" / "fixtures" / "eaws_CH.json"
...
class Command(BaseCommand):
    """Build apps/regions/fixtures/eaws_CH.json from EAWS micro-region source files.
    ...
    help = (
        "Build apps/regions/fixtures/eaws_CH.json from the vendored EAWS Switzerland "
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        existing_fixture = load_fixture(_CH_FIXTURE)
        ...
        entries = carry_forward_preserved_fields(_CH_FIXTURE, entries)
        changes = diff_against_existing(_CH_FIXTURE, entries)
        ...
        write_fixture(_CH_FIXTURE, entries)
