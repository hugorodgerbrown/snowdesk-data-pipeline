"""build_italy_fixture — build apps/regions/fixtures/eaws_IT.json from EAWS source data.
...
Usage:
    ...
    # Write apps/regions/fixtures/eaws_IT.json.
...
_ITALY_FIXTURE = _REPO_ROOT / "apps" / "regions" / "fixtures" / "eaws_IT.json"
...
class Command(BaseCommand):
    ...
    help = (
        "Build apps/regions/fixtures/eaws_IT.json from the vendored EAWS Italy "
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        entries = carry_forward_preserved_fields(_ITALY_FIXTURE, entries)
...
def _build_entries(eaws_dir: Path, region_codes: list[str]) -> list[dict[str, Any]]:
    ...
    ``apps.regions.fixture_utils``, supplying ``_build_region_entries`` as the
    per-code builder and ``"build_italy_fixture"`` as the log label.
