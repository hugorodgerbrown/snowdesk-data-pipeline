"""
apps/accounts/identity.py — The public-facing identifier for a signed-in user.
...
The house style here follows ``apps.core.idempotency._principal``: small
...
def user_identity(user: Any) -> str:
    ...
    ``apps.accounts.context_processors.nav_subscriptions`` already guards. This
...
def request_identity(request: HttpRequest) -> str:
    ...
    attribute at all — the same guard ``apps.core.idempotency._principal``
