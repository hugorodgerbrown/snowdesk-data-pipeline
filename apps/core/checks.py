...
# (E003 — BUILD_IDENTITY — retired with the substitution by SNOW-1025.)
...
def check_sw_cache_version_substitutable(
    ...
    """Verify ``sw.js`` still carries its substitutable assignment (SNOW-590).
    ...
    SNOW-933 added a second substitution, ``BUILD_IDENTITY``, probed here
    as E003. SNOW-1025 removed it — the update banner no longer names
    builds, and injecting one made the worker's bytes change on every
    deploy — so E003 is retired rather than reused.
    """
