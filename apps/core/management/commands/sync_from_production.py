...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        for spec in plan:
            ...
            # No transaction around the table: each batch commits on its
            # own, and reconnect-with-backoff lives in production_sync. See
            # sync_table's docstring for why rolling a whole table back was
            # both unnecessary and the thing that toppled the server.
            try:
                results.append(
                    sync_table(spec, id_maps=id_maps, since=since, commit=commit)
                )
            except ProductionSyncError as exc:
