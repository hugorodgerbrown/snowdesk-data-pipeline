...
- `/trips/partials/…` — create, edit, delete, join, leave, save-route
  ...
  handed the uuid. **`join` and `leave` are "Save this trip" and "Remove
  from my trips" to a reader since SNOW-848** — the endpoint names predate
  the relabelling — and `save-route` no longer has a rendering caller: a
  trip is saved whole. The trip PAGES
