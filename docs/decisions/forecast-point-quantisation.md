...
200)`). `resolve_forecast_cell()` always checks for a reusable nearby
...
**Consequences.** `resolve_forecast_cell()` mints a new row for every pin
...
`apps/weather/services/forecast_cells.py`, mirroring
...
`ForecastCellQuerySet.active()` filters to points with at least one live
