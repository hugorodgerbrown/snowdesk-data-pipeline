...
SNOW-895**. Its trigger had been removed long before, because it covered the
terrain the visitor had just tapped, and what remained was unreachable: nine
live calls into its teardown helpers that were all permanent no-ops.
...
pin or a favourite pin opens, the route detail sheet a saved route's line
opens (`#route-detail-sheet` — SNOW-973 moved it out of that popup), the
...
so the sheets that go through it need nothing of their own — which is how
the route detail sheet joined the matrix without an edit to any other
surface.
...
- **Rows come from `routes:list`**, which renders
  `routes/partials/_route_list_map.html` and the shared
  `includes/_ugc_panel_row.html` row (SNOW-803 retired the `?variant=`
  parameter along with the account page that needed it); the row
...
| `GET /api/resorts.geojson` | `api:resorts_geojson` | GeoJSON FeatureCollection of geocoded resorts (Points; `[lon, lat]` per RFC 7946); properties `id`, `name`, `region_id`, `needs_review`, `tier` (SNOW-543 — `CORE`/`STANDARD`/`MINOR`, the curated map-prominence verdict the pin radius is interpolated from). **`id` is `Resort.slug`, never the pk** (SNOW-796): it is what the resort popup and the `?resort=<slug>` deep link resolve on, and what `favourites.geojson` carries as `resort_slug` so the favourited-resort exclusion compares like with like. |
| `GET /api/regions.geojson` | `api:regions_geojson` | GeoJSON FeatureCollection from `Region.boundary` (L4 fixture regions); each feature has `properties.id` + `properties.name`, plus `properties.download` (SNOW-521 — see below) when the region has a precomputed offline-basemap size. **Caching (SNOW-902):** an ETag plus `public, max-age=300, stale-while-revalidate=86400` — the cached copy still answers instantly for a day and refreshes behind the request, and an `If-None-Match` (compared weakly, since `GZipMiddleware` marks a compressed ETag weak) gets a 304 rather than the geometry. An **empty** FeatureCollection gets neither the stale window nor SNOW-896's day-long server-side memo, and `sw.js` refuses to write it to the shell cache at all: an empty answer means this deployment has no geometry for that country yet, and pinning that is how a database that gained the FR/AT/IT fixtures went on serving "no regions". Same policy on `major-regions.geojson`, `sub-regions.geojson` and `region-basemap-tiles`. Server-side `cache.get_or_set` keyed on country, 24 h (SNOW-896 — it was the only map endpoint with no server-side cache, and it is both the largest payload the map fetches and on the boot critical path). |
| `GET /api/major-regions.geojson` | `api:major_regions_geojson` | GeoJSON FeatureCollection of L1 EAWS major regions (e.g. `CH-4`, `CH-5`) with `properties.id` + `properties.name`. Never carries `properties.download` — offline-basemap download is a MicroRegion-only feature (SNOW-521). Server-side cache keyed on country, 24 h (SNOW-896). |
| `GET /api/sub-regions.geojson` | `api:sub_regions_geojson` | GeoJSON FeatureCollection of L2 EAWS sub-regions (e.g. `CH-41`, `CH-42`) with `properties.id` + `properties.name`. Never carries `properties.download`, same as major-regions.geojson above. Server-side cache keyed on country, 24 h (SNOW-896). |
...
| `GET /api/area-content/?bbox=w,s,e,n` | `api:area_content` | `{"regions":[{"id","slug"},…],"weather":[{"short_id"},…]}` — what one rectangle contains, for an offline download's content plan (SNOW-953). The micro-regions whose bounding box the box touches (edge-inclusive, the same over-selecting rule `docs/decisions/inside-the-boundary-is-complete.md` states) and the `Location.objects.public()` rows inside it — **never `active()`**, which also reaches private favourite pins. **Date-free on purpose**: the client holds its own day window (today, `OFFLINE_CONTENT_PAST_DAYS` behind it, and the last published day ahead) and composes the bulletin URLs itself, so one answer serves every caller and every day. 400 for a bbox that is missing, non-numeric, not four parts, out of range or inverted (`w>e` / `s>n`) — an inverted box is rejected rather than normalised, since the two rectangles it could mean are different sets of bulletins. Replaces the client-side selection that made the map load all four countries' outlines first: 764 KB to discover ~55 KB of pages. Both reference indexes are memoised server-side for 24 h; the response takes `_geojson_response`'s ETag + `public, max-age=300, stale-while-revalidate=86400`, and the path is in `_POSTHOG_EXEMPT_PATHS` or `Vary: Cookie` would defeat it. |
| `GET /api/region/<region_id>/summary/` | `api:region_summary` | `{html, level}` — `html` is the server-rendered MapLibre Popup snippet (danger-rating chip + geographic breadcrumb); `level` is the rating string the JS uses to stamp `data-level` on the popup container for the border colour. Honours `?d=YYYY-MM-DD` so the popup can show any scrubbed-to date; returns 400 on a malformed value. **Consumed by `static/js/map_region_panel.js`** (SNOW-879), which fetches it for the region panel, caches the HTML per `regionId\|dateKey` and prefetches it on hover — so it is on the hot path for that surface, not spare. It said "currently unused by the client" until the 2026-09-10 map review (SNOW-892) checked: what had lost its trigger was `openRegionPopup`, the MapLibre popup that used to render the response, and SNOW-895 deleted that. The `html` is still popup markup because the panel renders the same partial. |
...
endpoint serves a stale copy while it revalidates (`max-age=300`,
`stale-while-revalidate=86400`, with an ETag — SNOW-902), so a returning
client can still be handed an old rectangle blob until that revalidation
lands. See [`offline-map.md`](offline-map.md) for the full
