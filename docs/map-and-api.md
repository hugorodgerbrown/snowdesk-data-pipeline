...
(`openRegionPopup`) is still implemented in `map.js` but has no trigger,
because it covered the terrain the visitor had just tapped. Its **endpoint**
did not go with it — `api:region_summary` is live, and `map_region_panel.js`
is what consumes it now (SNOW-879); only the popup that used to render the
response is unreachable. `/map/` permanently redirects
