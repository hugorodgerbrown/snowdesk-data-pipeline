...
| Never sampled | `Route.slope_samples is None` — nothing has looked at this route's terrain yet. Categorically different from a segment whose `unknown` is set, which means the terrain WAS asked and had no answer there. The GeoJSON keeps them apart by omitting the `slope` property entirely for the first and using a null angle for the second | `_compact_slope` in `apps/routes/views.py`; `segmentFeatures` in `static/js/route_slope_core.js` |
