...
| Rail | The horizontal bar below the map carrying one route unrolled against distance along it — its legs, its height profile, its steepness and its marks, all on one axis. The map answers where the route goes; the rail answers what is along it, and the two share one cursor position | Designed, not yet built — the name is registered here so the build starts on settled vocabulary |
