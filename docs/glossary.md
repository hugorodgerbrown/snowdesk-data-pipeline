...
| Leg | A stretch of a Route between two transitions, so it is one activity throughout — skinning, or skiing, or bootpacking. Derived from `points` alone on every read: no DEM, no network, no stored column, which is why it works offline and costs microseconds. The smoothing window is in METRES rather than in points, because a point is a position fix at whatever interval the recording device chose and the same setting would otherwise smooth away a roll on one track while preserving it on another (SNOW-990) | `Leg` / `detect_legs` in `apps/routes/services/legs.py` (SNOW-990). Nothing renders them yet |
| Transition | Where one Leg ends and the next begins — the moment the activity changes, which is the only thing on a route's line that is a decision rather than a description. There are N − 1 of them for N legs, and they are what the map names rather than the legs themselves | The boundary between consecutive `Leg` runs; `_runs` / `_merge_short_runs` in `apps/routes/services/legs.py` |
| Rail | The horizontal bar below the map carrying one route unrolled against distance along it — its legs, its height profile, its slope bands and its marks, all on one axis. The map answers where the route goes; the rail answers what is along it, and the two share one cursor position | Designed, not yet built — the name is registered here so the build starts on settled vocabulary |
...
| Slope band | A run of consecutive slope segments sharing one steepness class — variable length, where a slope segment is a fixed 25 m. It is the object the rail draws and a reader presses: "2.1 km at 40–45°" is a band, the 84 segments inside it are not. Not stored: derived per read from the same `segments` array, which is what lets the merge threshold be re-tuned without a migration | `slopeBands` in `static/js/elevation_profile_core.js`, which already computes exactly this for the elevation profile's stroke and had no name for it |
