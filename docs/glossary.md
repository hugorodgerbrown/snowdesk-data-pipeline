...
| Term | Meaning | Code |
|------|---------|------|
| WeatherSnapshot | Open-Meteo weather for one (region, date): WMO `weather_code` 0–99, sunrise/sunset, plus the nullable daily aggregates `temperature_2m_max` / `temperature_2m_min` (°C) and `snowfall_sum` (cm) added in SNOW-571 | `apps/bulletins/models.py`; fetched by `apps/weather/services/weather_fetcher.py` |
