...
| `apps.weather.services.weather_fetcher.fetch_weather_async` | `bulletin_detail` page render where no `WeatherSnapshot` exists for `(region, target_date)` and `target_date < today` | Idempotent DB pre-check, then `fetch_archive_for_region` / `fetch_weather_for_region` (Open-Meteo) | `WeatherSnapshot` row via `update_or_create` | `Exception` caught, logged at WARNING via `logger.warning(exc_info=True)`; `connections.close_all()` in `finally` so the thread releases its DB connection | `apps.weather.services.weather_fetcher` | `WEATHER_FETCH_ASYNC` (default True; tests pin False) |
