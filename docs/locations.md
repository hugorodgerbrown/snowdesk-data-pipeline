...
**`centroid_location` is durable, but only since the deploy stopped
reloading fixtures.** `bin/build.sh` used to run `loaddata` over the EAWS
fixtures on every deploy, and `loaddata` resets every column those fixtures
do not carry — so this FK was NULLed on all 461 regions on every deploy and
the orphaned `Location` rows accumulated (SNOW-771). The reload is gone, so
the link now persists.

Re-linking is `link_region_centroid_locations`, run by an operator after
seeding or a fixture change. It is offline — coordinate from `boundary`,
elevation from `MicroRegion.centroid_elevation_m`, both in the fixture — and
reuses the existing anonymous `Location` at the coordinate rather than
minting a new one, so re-running it never orphans weather. See
