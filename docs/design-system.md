...
- **EAWS glossary terms inside the prose.** *Shipped — SNOW-853.* When provider prose uses a term from the [EAWS glossary](https://www.avalanches.org/glossary/) — *avalanche prone location*, *weak layer*, *melt-freeze crust* — the word itself becomes the affordance: a dotted underline in `--color-text-3`, and a tap opens the standard's **verbatim** definition in a native `[popover]`. There is no JavaScript and no glossary page; the teaching happens where the question arises. The definition text is EAWS's, not Snowdesk's, and the popover links its own anchor on the source page to say so — the same provenance rule as the field-guidance credit above, one notch stricter, because a definition is quoted rather than paraphrased. The markup is spliced into the sanitised prose by `snowdesk_html`, not written in a template, so `.glossary-term` / `.glossary-def` / `.glossary-src` live in `main.css` as CSS component classes (the same exception `.slf-prose` takes) and `bin/ds-lint` cannot see them — token compliance there is a review-time check. `/_components/` documents the shape under **Glossary term in prose**. See [`docs/decisions/glossary-terms-injected-after-sanitisation.md`](decisions/glossary-terms-injected-after-sanitisation.md).
