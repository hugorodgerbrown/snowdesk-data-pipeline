...
| `apps/core/sw_shell.py` | SNOW-517, SNOW-590 — derives `CACHE_VERSION` from the shell content hash; `serve_sw` injects it per response. See "Cache version (derived)" below. SNOW-933 added a second injected constant, `BUILD_IDENTITY` (the serving deploy's git SHA and release label), which the worker hands back on a `build-identity` message so the update banner can name the build it is replacing. |
...
worker**, not the page (SNOW-933): `sw_register.js` posts
`{ type: 'build-identity' }` and reads the reply off a transferred
`MessagePort`, on a 2s budget, falling back to the page's
`pwa-app-version` meta when no worker answers. The worker replies from
`BUILD_IDENTITY`, injected per response by `serve_sw` beside
`CACHE_VERSION`. Reading the page instead left the banner permanently
unnumbered on staging — see
[`docs/decisions/the-update-banner-names-the-worker-being-replaced.md`](decisions/the-update-banner-names-the-worker-being-replaced.md).
