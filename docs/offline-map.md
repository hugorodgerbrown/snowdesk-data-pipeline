...
`sw_register.js` posts `{ type: 'SKIP_WAITING' }` to it the next time the
page is hidden (`visibilitychange` → `hidden`: a tab switch, an app switch,
a screen lock). It holds back while a `warmCache` run is queued or in
flight, so a basemap download is never cut off. On `activate` the worker
