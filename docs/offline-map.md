...
| `static/offline.html` | Branded fallback page returned by the SW when an HTML navigation fails AND no cached copy exists (SNOW-118). Inline-styled; one external asset, `static/js/pwa_reset.js`, which binds the §12.7 "Reset local data" panel this page carries because the manage page's copy is `@never_cache` (see [`offline-first.md`](offline-first.md#reset-local-data-snow-378)). |
...
`_isNoStore` (`static/js/sw.js:1506`) token-splits the header and looks
for the directive, because Cache Storage is not the HTTP cache —
`cache.put` honours no response header of its own. Django's
`@never_cache` is what puts authenticated HTML on the right side of that
gate: `manage_view` (`apps/accounts/views.py:1408`) and
`change_email_view` (`apps/accounts/views.py:699`) both carry it, and
`AdminSite.admin_view` already applies `never_cache` to every admin view
— which counts here because `templates/admin/base_site.html` registers
this same worker on the admin.
...
- `/account/manage/` and `/account/change-email/` are never available
  offline at all. `@never_cache` keeps them out of the shell cache, so
  there is nothing to serve. That takes the §12.7 "Reset local data"
  escape hatch and the `log:sync` read-out offline with it — see
...
- `tests/e2e/test_offline_account_principal.py` (SNOW-607, `tox -e e2e`)
  — the browser journey for the write side: `/account/manage/` never
  enters Cache Storage, and an ordinary navigation is cached carrying
  its stamp. Both assert Cache-Storage membership rather than inferring
  it from a failed fetch — `page.context.set_offline(True)` governs
  page-side network only, so a request the worker handles still reaches
