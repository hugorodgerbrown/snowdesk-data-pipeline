...
| `static/icons/pwa/splash/` | SNOW-878 — iOS launch screens (`apple-touch-startup-image`), one per device × orientation × colour scheme, plus the `splash-manifest.json` that `apps/public/pwa_splash.py` reads to emit the links. Generated by `bin/build-pwa-splash` from the built `icon-512.png`. Without these an iOS launch paints white; see "The launch screen" below. |
| `templates/includes/_pwa_launch_shell.html` | SNOW-878 — the in-app half of the splash, continuing the OS launch screen until the map is ready. Gated to cold launches of the installed app by an inline `display-mode: standalone` + `sessionStorage` check; dismissed by `static/js/pwa_launch_shell.js`, backstopped by a CSS failsafe. |
...
| `apps/core/sw_shell.py` | SNOW-517, SNOW-590 — derives `CACHE_VERSION` from the shell content hash; `serve_sw` injects it per response. See "Cache version (derived)" below. It is the only per-deploy value in the worker: SNOW-1025 removed SNOW-933's `BUILD_IDENTITY`, so the worker's bytes change only when the shell does, or (SNOW-1029, the release label is part of the name) on a new production release. |
| `apps.public.debug_views.sw_version` (`/_sw-version/`) | Staff-only page surfacing the deployed `CACHE_VERSION` / `APP_VERSION` alongside the live SW version reported by `static/js/pwa_sw_version_probe.js` (SNOW-517), and (SNOW-1027) every worker the registration holds (active, waiting, installing) with the `CACHE_VERSION` each reports. DevTools names workers only by Chrome's own counter. |

## Update strategy

A deploy asks nothing of the user (SNOW-1025,
[`service-worker-updates-apply-silently.md`](decisions/service-worker-updates-apply-silently.md)).

The SW deliberately does **not** call `self.skipWaiting()` on `install`.
A freshly installed worker sits in the `waiting` state, and
`sw_register.js` posts `{ type: 'SKIP_WAITING' }` to it:

- **at once** when the waiting worker reports (over `shell-identity`) the
  same shell as the page's `<meta name="pwa-shell">` (SNOW-1027). This is
  a fresh tab: the page came off the network as the new build;
...
`controllerchange` reloads only when the user pressed Reload on the banner
(the `userTriggeredUpdate` flag), so neither a first-install
`clients.claim()` nor a silent activation moves a page under the user, and
there is no dev reload loop. (This is a different concern from the
SNOW-585 dev shell-cache bypass below, which exists because the *previous*
worker can stay in control across a `git pull`. See "Dev shell-cache
bypass".)
...
1. `shellIsStale` (SNOW-952): the shell cache name the controlling worker
   ...
   would serve (`shell` on `/api/version`). See
...
2. `workerIsStuck` (SNOW-1025): after `registration.update()`, no worker
   is waiting, and any installing worker went `redundant` or did not
   finish within 30s. A worker that installs will be applied silently, so
   it never raises the banner.

`revealNow` is the ungated primitive. Calling it from a reveal path puts
the every-deploy interruption back. The copy is plain ("Snowdesk needs a
refresh / It couldn't update itself. Reload to finish.") and names no
build. SNOW-869's versioned copy and SNOW-933's controller-identity
labelling are gone.
...
- **Reload** — dispatched by `handleReloadClick` in `sw_register.js`. If a
  ...
  reloads once). If no worker is waiting (the stuck case), clears
...
### The launch screen (SNOW-878)

This section used to say that iOS splash images were deliberately
omitted, because "a launch on the brand `background_color` is
acceptable". That trade was never actually on offer: **iOS does not
fall back to `background_color`**. With no `apple-touch-startup-image`
whose media query matches the device exactly, a home-screen launch
paints plain **white** for the whole cold boot. Every iPhone install
launched to a blank screen for the project's whole life.

The launch is now covered in two halves, designed to be
indistinguishable from one another:

| Half | Owner | Covers |
|------|-------|--------|
| OS launch screen | `bin/build-pwa-splash` → `static/icons/pwa/splash/` | Tap → first paint |
| In-app launch shell | `templates/includes/_pwa_launch_shell.html` | First paint → map ready |

**The OS half.** `bin/build-pwa-splash` renders the page background with
the home-screen tile centred on it, once per device × orientation ×
colour scheme — 68 PNGs, ~1 MB, indexed-palette (the truecolour default
wrote the same images at 4.8 MB). It composites the *built*
`icon-512.png` rather than re-rendering `favicon.svg`, so the launch
screen cannot drift from the home-screen icon; run
`npm run build:icons` before `npm run build:splash`.

The device matrix lives in that script and nowhere else. It writes
`splash-manifest.json` beside the PNGs; `apps/public/pwa_splash.py`
reads it back and `templates/includes/_pwa_splash.html` emits one
`<link rel="apple-touch-startup-image">` per entry, site-wide (iOS
caches them when the user adds the app to the Home Screen, and they can
do that from any page). **Adding a device is one row in the script plus
a rebuild** — nothing on the Python side enumerates them, so a link can
never name a file that was never rendered.

Two things that look like bugs and are not:

- **`device-width` / `device-height` do not swap for landscape.** They
  describe the device in its natural orientation; the `orientation`
  term is what picks between the two images. Swapping them yields a
  query no device matches, and the launch silently reverts to white.
  `tests/public/test_pwa_splash.py` asserts the two orientation sets
  are identical.
- **Staging uses the production splash.** Unlike the icons (SNOW-399)
  there is no amber set. The staging tell has to be legible where a
  tester *chooses* the app — the home-screen tile and the status bar —
  and 68 more PNGs to tint a half-second launch screen is a poor trade.

**The in-app half.** The OS screen disappears at first paint, which on
the map page is well before there is a map: chrome paints while `#map`
is still empty, because MapLibre fetches its style and tiles with
`fetch()`. `_pwa_launch_shell.html` continues the same composition —
same background token, same tile, same size and position — until
`static/js/pwa_launch_shell.js` dismisses it on `snowdesk:map-ready`
(dispatched at the top of map.js's `load` handler), on `load` for pages
without a map, or on a 5s timeout. A CSS failsafe keyframe hides it at
10s whether or not that script ever parsed, because the failure mode of
an overlay that covers the whole app is an outage, not a cosmetic bug.

It shows only for a **cold launch of the installed app**: an inline gate
in the partial checks `display-mode: standalone` (plus iOS's legacy
`navigator.standalone`) and a `sessionStorage` flag that makes it
once-per-launch rather than once-per-navigation. The gate is inline
because it has to resolve before first paint — the same constraint as
`includes/theme_head.html`.

**Colour.** `--color-bg` is the single source: the splash PNGs, the
in-app shell and the manifest's `background_color` all carry it, so the
splash hands over to a page painting the identical colour.
SNOW-878 also fixed a drift here — the manifest declared `#f4f1e8`
(map.css's `--paper`) against a page painting `#f2f0ec`.
`tests/public/test_pwa_splash.py` reads the token out of
`src/css/main.css` and fails on any future drift.

The manifest has no per-scheme `background_color`, so **iOS is the only
platform whose launch screen follows the theme** (via
`prefers-color-scheme` on the links). Android keeps the light value. A
user who pins a theme in `localStorage` against their OS setting gets
the OS scheme on the launch screen; nothing can read that preference
...
stylesheets that page references, and — SNOW-912 — the `/api/` feeds its
own boot will ask for, derived from the `data-today` baked into the page
just warmed. Page and feeds move as one unit: warming the page alone left a
device that opened to a grey map with no ratings and no region outlines. Without the re-warm, every deploy left
...
RESET_SCRIPT]`, and SNOW-907's `AUDIT_SCRIPTS` are two more scripts, not
tiles). So for the project's whole life, an offline reader who
...
response with `snowdesk-shell-v<APP_RELEASE>-<first 12 hex of the shell
content hash>`, e.g. `snowdesk-shell-v34-a1b2c3d4e5f6` (SNOW-1029). An
unnumbered build (empty `APP_RELEASE`) drops the label:
`snowdesk-shell-a1b2c3d4e5f6`. The hash is what makes the name correct;
the label is only there so a person can tell a running worker from a
waiting one, on `/_sw-version/` or in DevTools → Cache Storage.
