...
| `apps/core/sw_shell.py` | SNOW-517, SNOW-590 — derives `CACHE_VERSION` from the shell content hash; `serve_sw` injects it per response. See "Cache version (derived)" below. It is the only per-deploy value in the worker: SNOW-1025 removed SNOW-933's `BUILD_IDENTITY`, so the worker's bytes change only when the shell does. |
