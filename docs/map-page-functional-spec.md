...
| **Bulletins** | derived | The bulletin data painted onto those regions: **the choropleth** — the coloured fill carrying the danger rating — plus the dissolved outer boundary of every micro-region that shares one bulletin on the selected day. Shows how the warning service actually grouped terrain that day: two regions inside one outline are covered by one text. | **On** |
...
**Why Micro regions and Bulletins are separate rows** (SNOW-656). The
...
polygons and is unreadable on top of a coloured fill. **Bulletins and the
...
Switching Bulletins off makes the fill *transparent*, not absent — it is the
layer region taps resolve against, so borders stay tappable with the colour
gone. Only with **both** rows off is it removed outright.
