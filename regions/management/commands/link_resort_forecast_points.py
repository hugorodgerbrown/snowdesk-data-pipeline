...
(``bulletins.services.forecast_points.resolve_forecast_point``). Widening
``ForecastPoint.objects.active()`` to favourite-OR-resort (see
``bulletins/models.py``) means the scheduled ``fetch_weather`` point pass
...
(mirroring ``favourites.services.create_favourite``) so a slow or failing
...
class Command(BaseCommand):
    ...
    help = (
        ...
        "via bulletins.services.forecast_points.resolve_forecast_point, "
